...
then
	...
	sed -e "s/VerSION/$VERSION/g" <gdb-mingw32ce-arm.spec.in >gdb-mingw32ce-arm.spec && \
	sed -e "s/VerSION/$VERSION/g" <gdb-mingw32ce-i386.spec.in >gdb-mingw32ce-i386.spec && \
	OK=yes
	if [ $OK = "no" ]; then
...
then
	...
	if [ ! -r /usr/src/rpm/RPMS/i586/gdb-mingw32ce-arm-$VERSION-1.i586.rpm ]; then
		echo "RPM doesn't appear to be there, exiting ..."
		echo "ls -l /usr/src/rpm/RPMS/i586/gdb-mingw32ce-arm-$VERSION-1.i586.rpm"
		exit 1
	fi
	#
...
then
	...
	if [ ! -r /usr/src/rpm/RPMS/i586/gdb-mingw32ce-i386-$VERSION-1.i586.rpm ]; then
		echo "RPM doesn't appear to be there, exiting ..."
		echo "ls -l /usr/src/rpm/RPMS/i586/gdb-mingw32ce-i386-$VERSION-1.i586.rpm"
		exit 1
	fi
	#
...
then
	...
	# Gdb
	#
	LIST=`ls gdb-mingw32ce-arm-$VERSION-?.i586.rpm`
	COUNT=`echo $LIST | wc -w`
	if [ $COUNT -ne 1 ]; then
		echo "Unexpected number of RPM files, please clean up before continuing."
		exit 1
	...
	echo "put $LIST $PREFIX$LIST" >>$FTPFILE
	LIST=`ls gdb-mingw32ce-i386-$VERSION-?.i586.rpm`
	COUNT=`echo $LIST | wc -w`
	if [ $COUNT -ne 1 ]; then
		echo "Unexpected number of RPM files, please clean up before continuing."
		exit 1
	...
	echo "put $LIST $PREFIX$LIST" >>$FTPFILE
	echo "quit" >>$FTPFILE
	sftp -b $FTPFILE dannybackx@frs.sourceforge.net
	#
...
#	echo "put cegcc-mingw32ce-src-$VERSION.tar.gz" >>$FTPFILE
	echo "quit" >>$FTPFILE
	OK=no
	sftp -b $FTPFILE dannybackx@frs.sourceforge.net && OK=yes
	if [ $OK = "no" ]; then
