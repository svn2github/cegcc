...
static bool arm_pe_dllexport_p (tree);
static bool arm_pe_dllimport_p (tree);
static void arm_pe_mark_dllexport (tree);
static void arm_pe_mark_dllimport (tree);
...
static void
arm_pe_mark_dllexport (tree decl)
{
  const char *oldname;
  char  *newname;
  rtx rtlname;
...
//  SYMBOL_REF_DECL (symref) = decl;	/* Danny */
...
static void
arm_pe_mark_dllimport (tree decl)
{
  const char *oldname;
  char  *newname;
  tree idp;
...
arm_pe_encode_section_info (tree decl, rtx rtl, int first ATTRIBUTE_UNUSED)
{
  ...
  if (arm_pe_dllexport_p (decl))
    arm_pe_mark_dllexport (decl);
  else if (arm_pe_dllimport_p (decl))
    arm_pe_mark_dllimport (decl);
  /* It might be that DECL has already been marked as dllimport, but a
     ...
     DECL_RTL still has @i.__imp_foo.  We need to remove that.  */
	   ...
	   && arm_pe_dllimport_name_p (XSTR (XEXP (XEXP (DECL_RTL (decl), 0), 0), 0)))
    {
