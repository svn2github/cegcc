...
Void_t* public_mALLOc(size_t bytes) {
  ...
  m = mALLOc(bytes);
  WCETRACE(WCE_MALLOC, "m: %d %p", bytes, m);
...
Void_t* public_rEALLOc(Void_t* m, size_t bytes) {
  ...
  m = rEALLOc(m, bytes);
  WCETRACE(WCE_MALLOC, "r: %d %p", bytes, m);
  if (MALLOC_POSTACTION != 0) {
  }
  return m;
}
...
Void_t* public_mEMALIGn(size_t alignment, size_t bytes) {
  ...
  WCETRACE(WCE_MALLOC, "ma: %d %d", alignment, bytes);
  if (MALLOC_POSTACTION != 0) {
  }
  return m;
}
...
Void_t* public_vALLOc(size_t bytes) {
  ...
  m = vALLOc(bytes);
  WCETRACE(WCE_MALLOC, "valloc: %d %p", bytes, m); 
  if (MALLOC_POSTACTION != 0) {
  }
  return m;
}
