...
static void show_errmsg_dll(const char *func)
{
	...
	MessageBoxW(0, w_msg, NULL, MB_OK | MB_SETFOREGROUND);
	free(msg);
	free(w_msg);
}
...
void doit(const TCHAR *text)
{
	...
	MessageBoxW(0, buf, text, MB_OK | MB_SETFOREGROUND);
#else
	MessageBoxW(0, L"Message from DLL", text, MB_OK | MB_SETFOREGROUND);
#endif
