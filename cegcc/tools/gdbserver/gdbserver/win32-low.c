		...
		DWORD flags, PROCESS_INFORMATION *pi)
{
  ...
  ret = CreateProcessW (wprogram, /* image name */
                        ...
                        pi);      /* proc info */
  OUTMSG2(("CreateProcessW -> %d, error %d, process %d\n", ret, GetLastError(), pi->dwProcessId));
#else
...
{
  if (current_process_handle == NULL) {
    gui_status("win32_kill: no process");
    return;
  }
  ...
  for (;;)
	...
	gui_status ("win32_kill: DBG_CONTINUE");
	break;
      }
      if (!WaitForDebugEvent (&current_event, INFINITE)) {
	gui_status ("WaitForDebugEvent -> break\n");
	break;
      }
...
get_child_debug_event (struct target_waitstatus *ourstatus)
{
  ...
  if (attaching)
	...
	if (e == ERROR_PIPE_NOT_CONNECTED) {
		OUTMSG2(("ERROR_PIPE_NOT_CONNECTED\n"));
